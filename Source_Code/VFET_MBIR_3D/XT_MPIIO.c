...
int32_t read_SharedBinFile_At (char filename[100], Real_arr_t* data, int32_t offset, int32_t size, FILE* debug_file_ptr)
{
	...
	ret = MPI_File_open(MPI_COMM_WORLD, BinFilename, MPI_MODE_RDONLY, MPI_INFO_NULL, &fh);
	if (ret != 0) {fprintf(debug_file_ptr, "ERROR: Cannot Read file %s ....\n", BinFilename); return(-1);}
