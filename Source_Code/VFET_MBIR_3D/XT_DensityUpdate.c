...
void compute_crossprodtran (Real_arr_t**** magobjinp, Real_arr_t*** elecobjinp, Real_arr_t**** magobjout, Real_arr_t*** elecobjout, Real_arr_t**** magimp, Real_arr_t**** elecimp, FFTStruct *fftptr, int32_t N_z, int32_t N_y, int32_t N_x, Real_t tran_sign)
{
	...
	Real_t real_xz, real_xy, real_yz, real_yx, real_zx, real_zy, imag_xz, imag_xy, imag_yz, imag_yx, imag_zx, imag_zy, N, real_rho, imag_rho;
...
void compute_crossprodstep (Real_arr_t**** magobjinp, Real_arr_t*** elecobjinp, Real_arr_t**** magimp, Real_arr_t**** elecimp, FFTStruct *fftptr, int32_t N_z, int32_t N_y, int32_t N_x, Real_t* alpha_mag, Real_t* alpha_elec)
{
	int32_t i, j, k;
	int64_t idx, idx_i, idx_j, idx_k;
	Real_t real_1, imag_1, real_2, imag_2, real_x, imag_x, real_y, imag_y, real_z, imag_z, real_rho, imag_rho, N;
...
void compute_gradient_stepsize(ScannedObject* ObjPtr, TomoInputs* InpPtr, FFTStruct* fftptr, Real_arr_t**** grad_mag, Real_arr_t*** grad_elec, Real_t* alpha_mag, Real_t* alpha_elec)
{
    	...
    	Real_t VMag[3], VElec, temp_mag[3][3][3][3], temp_elec[3][3][3], alpha_mag_temp, alpha_elec_temp, Delta0_Mag[3], Delta0_Elec, QGGMRF_Params_Mag[3], QGGMRF_Params_Elec, gradmagavg[3], gradelecavg, gradmagforw[3], gradelecforw;
	Real_arr_t ****NeighAlphaMag, ***NeighAlphaElec;

	*alpha_mag = 0; *alpha_elec = 0;

	compute_crossprodtran (ObjPtr->ErrorPotMag, ObjPtr->ErrorPotElec, grad_mag, grad_elec, ObjPtr->MagFilt, ObjPtr->ElecFilt, fftptr, ObjPtr->N_z, ObjPtr->N_y, ObjPtr->N_x, -1.0);
	...
	gradmagavg[0] = 0; gradmagavg[1] = 0; gradmagavg[2] = 0; gradelecavg = 0;	
	gradmagforw[0] = 0; gradmagforw[1] = 0; gradmagforw[2] = 0; gradelecforw = 0;	
	...
	compute_crossprodstep (grad_mag, grad_elec, ObjPtr->MagFilt, ObjPtr->ElecFilt, fftptr, ObjPtr->N_z, ObjPtr->N_y, ObjPtr->N_x, &alpha_mag_temp, &alpha_elec_temp);
	...
	*alpha_elec = alpha_elec_temp/(*alpha_elec + EPSILON_ERROR);
	...
	fprintf(InpPtr->debug_file_ptr, "Average of absolute value of gradient is (x, y, z, elec) = (%e, %e, %e, %e) and stepsize is (mag, elec) = (%e, %e)\n", gradmagavg[0], gradmagavg[1], gradmagavg[2], gradelecavg, *alpha_mag, *alpha_elec);
	fprintf(InpPtr->debug_file_ptr, "Average of absolute value of forward part of gradient is (x, y, z, elec) = (%e, %e, %e, %e)\n", gradmagforw[0], gradmagforw[1], gradmagforw[2], gradelecforw);
